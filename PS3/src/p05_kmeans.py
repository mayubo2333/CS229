...
def init_centroids(num_clusters, image):
    ...
    selected_idx = random.sample(range(image.shape[0]*image.shape[1]), num_clusters)
    selected_x = np.array(list(idx//image.shape[1] for idx in selected_idx))
    selected_y =  np.array(list(idx%image.shape[1] for idx in selected_idx))
    centroids_init = np.array(image[selected_x, selected_y])
    print("Centroid shape: {}".format(centroids_init.shape))
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    iter = 0
    while iter<max_iter:
        batch_image = np.array(image.reshape(1, image.shape[0], image.shape[1], image.shape[2]), dtype=int)
        batch_centroids = np.array(centroids.reshape(centroids.shape[0], 1, 1, centroids.shape[1]), dtype=int)
        batch_dist_square = np.sum((batch_image-batch_centroids)**2, axis=-1)
        batch_class = np.argmin(batch_dist_square, axis=0)

        new_centroids = np.zeros((centroids.shape[0], image.shape[-1]))
        for k in range(centroids.shape[0]):
            new_centroids[k] = np.mean(image[batch_class==k], axis=0)
...
def update_image(image, centroids):
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            image[i, j] = centroids[batch_class[i, j]]
    # *** END YOUR CODE ***
...
def main(args):
    ...
    # Load small image
    image = np.copy(cv2.imread(image_path_small))
    ...
    # Load large image
    image = np.copy(cv2.imread(image_path_large))
