...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    s = forward_softmax(x)
    ds_dx = np.diag(s) - s.reshape(-1, 1)@s.reshape(1,-1)
    grads = ds_dx.T @ grad_outputs
    return grads
...
def backward_relu(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    dh_dx = np.array(x>=0, dtype=float)
    grads = dh_dx * grad_outputs
    return grads
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    dl_dw = forward_convolution(output_grad[:, None, :], np.zeros_like(conv_b), data)
    dl_db = np.sum(output_grad, axis=(-2, -1))

    P, _, U, V = conv_W.shape
    M, N = data.shape[-2:]
    U_tilde, V_tilde = M-U+1, N-V+1
    W_tilde = np.zeros((P, 2*M-U, 2*N-V))
    W_tilde[:, (M-U):M, (N-U):N] = conv_W[:, 0, :, :]
    reverse_output_grad = output_grad[:, ::-1, ::-1]

    dl_dx = np.zeros_like(data)
    for x in range(M):
        for y in range(N):
            dl_dx[:, x, y] = np.sum(
                np.multiply(W_tilde[:,x:(x+U_tilde), y:(y+V_tilde)], reverse_output_grad)
            )
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    # *** START CODE HERE ***
    P, M, N = output_grad.shape
    dl_dx = np.zeros_like(data)

    for channel in range(P):
        for x in range(M):
            for y in range(N):
                local_data = data[channel, x*pool_width:(x+1)*pool_width, y*pool_height:(y+1)*pool_height]
                row, col = np.where(local_data == local_data.max())
                row = row[0]; col = col[0]
                dl_dx[channel, x*pool_width+row, y*pool_height+col] = output_grad[channel, x, y]
    return dl_dx  
    # *** END CODE HERE ***
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    dl_dw = data.reshape(-1,1)@output_grad.reshape(1, -1)
    dl_db = output_grad
    dl_dx = (weights@output_grad.reshape(-1, 1)).T
    return (dl_dw, dl_db, dl_dx)
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    grad_outputs = backward_cross_entropy_loss(probabilities, labels) 
    grad_outputs = backward_softmax(logits, grad_outputs)
    dl_dw2, dl_db2, grad_outputs = backward_linear(W2, b2, flattened, grad_outputs)
    grad_outputs = np.reshape(grad_outputs, first_after_relu.shape)
    grad_outputs = backward_relu(first_max_pool, grad_outputs)
    grad_outputs = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_outputs)
    dl_dw1, dl_db1, grad_outputs = backward_convolution(W1, b1, data, grad_outputs)
