...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    prob_state = mdp_data['transition_probs'][state]
    expected_values = mdp_data['value'] @ prob_state
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    for i, p_count in enumerate(mdp_data['transition_counts']):
        if np.sum(p_count) != 0:
            if np.sum(p_count[:,0]) !=0 :
                mdp_data['transition_probs'][i,:,0] = (p_count[:,0])/np.sum(p_count[:,0])
            if np.sum(p_count[:,1]) !=0 :
                mdp_data['transition_probs'][i,:,1] = (p_count[:,1])/np.sum(p_count[:,1])
 
    mdp_data['reward'] = mdp_data['reward_counts'][:, 0]
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        new_value = mdp_data['reward'] + gamma * np.max(\
            np.squeeze(mdp_data['transition_probs'].transpose(0, 2, 1)@old_value.reshape(-1,1), axis=-1), axis=-1
        )
        mdp_data['value'] = new_value

        if np.max(np.abs(old_value-new_value))<tolerance:
            break
        iter += 1      
...
def main(seed, plot=True): 
    ...
    if plot:
        ...
        plt.savefig('./control_{}.png'.format(seed))
